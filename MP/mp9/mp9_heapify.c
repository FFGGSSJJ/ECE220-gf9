          ...
          path_t* path)
{
    ...
    int32_t path_start, path_end, path_forward[MAX_PATH_LENGTH];   // the id of the end and start of the path in graph
    int32_t* h_ptr = h->elt;
    ...
    path->n_vertices += 1;
    for (int i = path->n_vertices - 1, j = 0; i >= 0; i--, j++){
        path_forward[j] = path->id[i];
    }
    for (int i = 0; i < path->n_vertices; i++){
        path->id[i] = path_forward[i];
    }
