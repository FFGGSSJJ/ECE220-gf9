	...
	; R1 -- temporary
	; R2 -- latest value store in the stack
	; R3 -- 
	; R4 -- 
	; R5 -- 
	...
	AND R1,R1,#0
	STR R1,R6,#0	;store a special value
	; Write your program here.  	
READIN
	JSR TOKENIZE	;Read value into R0
	AND R1,R1,#0
	ADD R1,R0,#4
	BRz READIN		;if R0 = -4, ignore it
	ADD R0,R0,#0
	BRn JUDGE_CLOSE	;if R0 < 0, judge whether there exists corresponded 
	ADD R0,R0,#0
	BRp PUSHIN
	ADD R0,R0,#0
	BRz JUDGE_ENTER
...
JUDGE_CLOSE
	LDR R2,R6,#0	;Load the lastest value in stack into R2
	BRz PRINT_NOOPEN;If it is at the base, NOOPEN
	ADD R1,R0,R2	
	BRnp PRINT_MISMAT ; If it dose not match the lastest value in stack, MISMAT
	BRz	POPOUT		;If it matches, pop the lastest one out of stack
...
JUDGE_ENTER
	...
	BRnp PRINT_INVALID
PUSHIN
	ADD R6,R6,#-1	;Point to the next stack rather than base
	STR R0,R6,#0	;Store the current value to the next position
	BRnzp READIN

POPOUT		
	ADD R6,R6,#1	;Put the pointer back to the last position to pop it out
	BRnzp READIN
